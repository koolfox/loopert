...
async def main():
    agent = Agent(
        ...
        extend_system_message="Plan and act step-by-step. At each step: (1) wait for page load/idle, (2) gather DOM/rendered info, (3) check for new or delayed popups/consent/cookie banners, (4) clear blockers first (reject/close preferred), (5) execute the planned action, (6) verify outcome (URL/element present/screenshot). If any obstacle appears, run an obstacle handler (close popup, refresh+wait, scroll) before continuing. Maintain focus on the stated goal; do not wander. NEVER click Sign in / Log in / account prompts; treat auth pages (e.g., accounts.google.com) as blockers: go back or close them and return to the task. Do not enter or request credentials. Multi-language popup hints: reject/decline/refuse/no/later and accept/agree/allow/consent/continue in fr/de/es/it/pt/nl/no/sv/da/fi/ru/ja/ko/zh. Use keyboard (Tab/Enter/Esc) or ✕ when unclear. If captcha/human verification or persistent blockers occur, stop and report; when supervised, call ask_human for assistance/validation after a stuck step. Only take screenshots when page or URL changes. If supervised lead mode is True, defer to ask_human before first navigation and whenever uncertain; learn from the human’s narrated steps and then continue automatically.",
    )
    await agent.run()
